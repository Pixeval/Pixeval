        ...
        public class Illust : IParser<Illustration>
        {
            ...
            public Illustration Parse()
            {
                return new Illustration
                {
                    ...
                    Origin = MetaSinglePage.OriginalImageUrl.IsNullOrEmpty() ? MetaPages[0].ImageUrls.Large : MetaSinglePage.OriginalImageUrl,
                    Tags = Tags.Select(t => t.Name).ToArray(),
                    Thumbnail = ImageUrls.Medium,
                    Title = Title,
                    Type = Type == "illust" ? "illustration" : Type,
                    UserId = User.Id.ToString(),
                    UserName = User.Name,
                    PublishDate = CreateDate
                }.Apply(i =>
                {
                    i.TypeEnum = IllustTypeParser.ParseType(i);
                    if (!MetaPages.IsNullOrEmpty())
                    {
