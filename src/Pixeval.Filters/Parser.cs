...
public class Parser
{
    ...
    {
        var tok = Peek ?? throw new FilterParseException(FilterParseException.ErrorType.FilterTokenFinished, T.Name);
        if (tok is T t)
        {
            ++Position;
            return t;
        ...
        throw new FilterParseException(FilterParseException.ErrorType.UnexpectedToken, T.Name, tok?.ToString());
    }
    ...
    private LeafSequence Parse(out NumericRangeLeaf? index)
    {
        ...
        ParseArgumentList();
        index = _index;
        return Peek is null ? _queryTokenTree : throw new FilterParseException(FilterParseException.ErrorType.UnbalancedPar, Peek.ToString());
    }
    ...
    private void ParseArgumentList()
    {
        while (true)
        {
            ...
            if (Position > Tokens.Count)
                throw new FilterParseException(FilterParseException.ErrorType.ParserOutOfRange);
    ...
    private bool ParseArgument()
    {
        ...
        switch (Peek)
        {
            ...
            case IQueryToken.LeftParen:
            {
                ...
                switch (Peek)
                {
                    ...
                    default:
                        throw new FilterParseException(FilterParseException.ErrorType.ExpectedAndOrAfterLeftPar, Peek.ToString());
                }
            ...
            case IQueryToken.Index:
            {
                if (_index is not null)
                    throw new FilterParseException(FilterParseException.ErrorType.IndexRangeUsedMoreThanOnce);
        ...
        void EatBool(bool isExclude)
        {
            ...
            var type = content.Value.ToLowerInvariant() switch
            {
                ...
                "ai" => BoolType.Ai,
                "gif" => BoolType.Gif,
                _ => throw new FilterParseException(FilterParseException.ErrorType.InvalidConstraint, "r18, r18g, ai, gif", content.Value)
            };
        ...
        (long, long) ParseIntervalForm()
        {
            ...
            switch (Peek)
            {
                ...
                case IQueryToken.RightParen:
                    ...
                    break;
                default:
                    throw new FilterParseException(FilterParseException.ErrorType.ExpectedRightBracketOrParenInRange, Peek.ToString());
            }
            ...
            if (rightInclusive)
                b -= 1;
            if (a < 0)
                throw new FilterParseException(FilterParseException.ErrorType.NumericTooSmallInRange, oa.ToString());
            if (a > b)
                throw new FilterParseException(FilterParseException.ErrorType.MinimumShouldBeSmallerThanMaximum, oa.ToString(), ob.ToString());
            ...
            switch (Peek)
            {
                ...
                case IQueryToken.Numeric:
                {
                    ...
                    if (Peek is IQueryToken.Numeric)
                    {
                        var b = ParseIntegerNumber();
                        if (a > b)
                            throw new FilterParseException(FilterParseException.ErrorType.MinimumShouldBeSmallerThanMaximum, oa.ToString(), b.ToString());
                        return (a, b);
                    }
        ...
        switch (Peek)
        {
            ...
            case IQueryToken.Numeric:
            {
                ...
                if (Peek is IQueryToken.Numeric)
                {
                    var b = ParseDecimalNumber();
                    if (a > b)
                        throw new FilterParseException(FilterParseException.ErrorType.MinimumShouldBeSmallerThanMaximum, a.ToString(CultureInfo.CurrentCulture), b.ToString(CultureInfo.CurrentCulture));
                    range = (a, b);
                    break;
                }
    ...
    private (int?, int, int) ParseDateDesc()
    {
        ...
        throw new FilterParseException(FilterParseException.ErrorType.ExpectedAtLeastTwoNumericInDate, firstPart.ToString());
    ...
    public static int TryNarrow(long value)
    {
        if (value > int.MaxValue)
            throw new FilterParseException(FilterParseException.ErrorType.NumericTooLarge, value.ToString());
        return (int) value;
    }
}
