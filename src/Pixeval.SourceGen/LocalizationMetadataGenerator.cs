...
{
    public static ClassDeclarationSyntax GetClassDeclaration(string typeFullName, string className, Dictionary<string, List<Son>> members, bool isPartial)
    {
        const string symbolComboBoxItemName = "global::Pixeval.Controls.SymbolComboBoxItem";
        const string symbolTypeName = "global::FluentIcons.Common.Symbol";
        const string getLocalizationKey = "GetLocalizationKey";
        var distinctMembers = members.SelectMany(kvp => kvp.Value).Distinct().ToArray();
        const string getResource = "GetResource";
        const string i18NManager = "global::Pixeval.I18N.I18NManager";
        return ClassDeclaration(className)
            ...
            .AddMembers(ExtensionBlockDeclaration()
                    .AddParameterListParameters(Parameter(Identifier(i18NManager)))
                    .WithOpenBraceToken(Token(SyntaxKind.OpenBraceToken))
                    .WithCloseBraceToken(Token(SyntaxKind.CloseBraceToken))
                    .AddMembers(
                                ...
                                ArrowExpressionClause(
                                    InvocationExpression(
                                            MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression,
                                                IdentifierName(i18NManager), IdentifierName(getResource)))
            ...
            .AddMembers([
                .. members.Select(member =>
                    PropertyDeclaration(
                            ParseTypeName(
                                "global::System.Collections.Generic.IReadOnlyList<" + symbolComboBoxItemName + ">"),
                            member.Key + "Items")
                        .AddModifiers(Token(SyntaxKind.PublicKeyword), Token(SyntaxKind.StaticKeyword))
                        .WithExpressionBody(ArrowExpressionClause(CollectionExpression()
                            ...
                            [
                                .. member.Value.Select(CollectionElementSyntax (t) => ExpressionElement(
                                    ObjectCreationExpression(ParseTypeName(symbolComboBoxItemName))
                                        .AddArgumentListArguments(Argument(IdentifierName(t.FieldName)),
                                            Argument(
                                                InvocationExpression(MemberAccessExpression(
                                                        SyntaxKind.SimpleMemberAccessExpression,
                                                        IdentifierName(i18NManager),
                                                        IdentifierName(getResource)))
                                                    .AddArgumentListArguments(
                                                        Argument(IdentifierName(t.FieldName)))),
                                            Argument(
                                                CastExpression(
                                                    ParseTypeName(symbolTypeName),
                                                    LiteralExpression(SyntaxKind.NumericLiteralExpression, Literal(t.SymbolValue)))))))
                            ])))
                        .WithSemicolonToken(Token(SyntaxKind.SemicolonToken)))
            ]);
    }
...
public class LocalizationMetadataGenerator : LocalizationMetadataGeneratorBase, IIncrementalGenerator
{
    ...
    {
        if (attributeList is not [{ } first])
            return null;

        var isPartial = first is { NamedArguments: [{ Key: "IsPartial", Value.Value: true }] };
        ...
        foreach (var member in typeSymbol.GetMembers())
        {
            ...
            var attr = field.GetAttribute(SubAttributeFullName);
            if (attr is null)
                continue;
            ...
            {
                case [{ Value: int sv }, { Value: string rn }]:
                    symbolValue = sv;
                    resourceName = rn;
                    break;
                case [{ Value: string rn }]:
                    resourceName = rn;
                    break;
            }
            ...
            members.Add(new(typeFullName + "." + field.Name, resourceName, symbolValue));
        }

        var generatedType = GetClassDeclaration(typeFullName, typeSymbol.Name + "Extension", new() { [""] = members }, isPartial);
...
public class AttachedLocalizationMetadataGenerator : LocalizationMetadataGeneratorBase, IIncrementalGenerator
{
    ...
    {
        Dictionary<string, Dictionary<string, List<Son>>> subAttributesMap = [];
        ...
        foreach (var attribute in typeSymbol.GetAttributes())
        {
            ...
            if (attribute is
                {
                    ConstructorArguments: [{ Value: string distinctName }, ..],
                    AttributeClass.TypeArguments: [INamedTypeSymbol attachedType, ..]
                })
            {
                var f = attachedType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
                currentFather = f;
            ...
            if (attributeName == SubAttributeFullName)
            {
                int symbolValue = 0;
                ...
                {
                    case [{ Value: int sv }, { Value: string fn }, { Value: string rn }]:
                        symbolValue = sv;
                        fieldName = fn;
                        resourceName = rn;
                        break;
                    case [{ Value: string fn }, { Value: string rn }]:
                        fieldName = fn;
                        resourceName = rn;
                        break;
                }

                if (fieldName is not null && resourceName is not null)
                    current.Add(new(father + "." + fieldName, resourceName, symbolValue));
            }
        }

        foreach (var kvp in subAttributesMap)
        {
            var generatedType = GetClassDeclaration(kvp.Key, typeSymbol.Name, kvp.Value, true);
            var generatedNamespace = GetFileScopedNamespaceDeclaration(typeSymbol, generatedType, true);
            var compilationUnit = CompilationUnit()
                .AddMembers(generatedNamespace)
                .NormalizeWhitespace();
            yield return (kvp.Key.Split('.')[^1],
...
public struct Son(string fieldName, string resourceName, int symbolValue)
    : IEquatable<Son>
{
    ...
    public int SymbolValue { get; } = symbolValue;

    /// <inheritdoc />
    public bool Equals(Son other) => FieldName == other.FieldName;

    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is Son other && Equals(other);
