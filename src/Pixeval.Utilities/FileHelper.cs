...
public static class FileHelper
{
    ...
    extension(File)
    {
        public static FileStream OpenWriteOrTruncate(string path, int bufferSize = 4096)
        {
            ...
            return new FileStream(path, fileMode, FileAccess.Write, FileShare.None, bufferSize, false);
        }
        ...
        {
            var fileMode = File.Exists(path) ? FileMode.Truncate : FileMode.CreateNew;
            return new FileStream(path, fileMode, FileAccess.Write, FileShare.None, bufferSize, true);
        }
