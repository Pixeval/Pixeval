...
在下载作品时，这些宏会被替换为对应的文本，例如，`@{id}` 宏会在下载时被自动替换为作品 ID。
...
例如：`@{pic_set_index:000}` 可以将图片集索引格式化为三位数，不足三位的前面补零（000、001...）。
...
> [!WARNING]
> 若格式化含有路径中不支持的字符（如 \ / : * ? " < > | 等），则会自动删除，甚至可能到下载时才出现异常。
...
例如如果正在下载一个小说作品，则 `@{is_novel?小说:图片}` 会被替换为 `小说`；如果不是小说作品，则会替换为 `图片`。
