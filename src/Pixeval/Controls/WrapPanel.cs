...
public class WrapPanel : Panel, INavigableContainer, IOrientationBasedMeasures
{
    ...
    public static readonly StyledProperty<WrapPanelItemsAlignment> ItemsAlignmentProperty =
        AvaloniaProperty.Register<WrapPanel, WrapPanelItemsAlignment>(nameof(ItemsAlignment),
            defaultValue: WrapPanelItemsAlignment.Start);
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        for (int i = 0; i < children.Count; ++i)
        {
            ...
            var nextSpacing = itemExists && child.IsVisible ? itemSpacing : 0;
            if (MathUtilities.GreaterThan(this.Minor(curLineSize) + this.Minor(childSize) + nextSpacing,
                    this.Minor(finalSize))) // Need to switch to another line
            {
        ...
        void ArrangeLine(double lineMajor, int start, int endExcluded)
        {
            ...
            switch (tempItemsAlignment)
            {
                ...
                case WrapPanelItemsAlignment.Justify:
                    var totalMinorSpacing =
                        this.Minor(finalSize) - totalMinor - 0.01; // small epsilon to avoid rounding issues
                ...
                case WrapPanelItemsAlignment.Stretch /*or WrapPanelItemsAlignment.StretchAll*/:
                    var finalMinorWithoutSpacing =
                        this.Minor(finalSize) - minorSpacing * minorSpacingCount -
                        0.01; // small epsilon to avoid rounding issues
...
internal static class OrientationBasedMeasuresExt
{
    extension(IOrientationBasedMeasures m)
    {
        ...
        public Point MinorMajorPoint(double minor, double major) =>
            m.IsVertical ? new Point(minor, major) : new Point(major, minor);

        public Size MinorMajorSize(double minor, double major) =>
            m.IsVertical ? new Size(minor, major) : new Size(major, minor);
    }
}
