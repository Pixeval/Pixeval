...
// 
// Pixeval/Pixeval
// Copyright (c) 2021 Pixeval/ImageInline.cs
// 
...
namespace Pixeval.CommunityToolkit.Markdown.Parsers.Markdown.Inlines
{
    ...
    public class ImageInline : MarkdownInline, IInlineLeaf
    {
        ...
        /// <summary>
        ///     Attempts to parse an image e.g. "![Toolkit
        ///     logo](https://raw.githubusercontent.com/CommunityToolkit/WindowsCommunityToolkit/master/CommunityToolkit.WinUI.SampleApp/Assets/ToolkitLogo.png)".
        /// </summary>
        ...
        internal static InlineParseResult? Parse(string markdown, int start, int end)
        {
            ...
            // Extract the alt.
            var tooltip = markdown.Substring(start + 2, pos - (start + 2));
        ...
        public override string ToString()
        {
            ...
            {
                > 0 when ImageHeight > 0 => $"![{Tooltip}]: {Url} (Width: {ImageWidth}, Height: {ImageHeight})",
                > 0 => $"![{Tooltip}]: {Url} (Width: {ImageWidth})",
                _ => ImageHeight > 0 ? $"![{Tooltip}]: {Url} (Height: {ImageHeight})" : $"![{Tooltip}]: {Url}"
            };
        }
    }
}