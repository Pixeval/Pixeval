        ...
        private class UserPreviewAsyncEnumerator : AbstractPixivAsyncEnumerator<User>
        {
            ...
            protected override void UpdateEnumerator()
            {
                userPreviewEnumerator = entity.UserPreviews.NonNull().Select(u => new User
                {
                    ...
                    Thumbnails = EnumerableExtension.NonNull<UserNavResponse.Illust>(u.Illusts).Select(_ => _.ImageUrl.SquareMedium).ToArray(),
            ...
            private static async Task<ResponseBase<UserNavResponse>> TryGetResponse(string url)
            {
                var res = (await HttpClientFactory.AppApiHttpClient().GetStringAsync(url)).FromJson<UserNavResponse>();
                if (res is { } response && !response.UserPreviews.IsNullOrEmpty()) return ResponseBase<UserNavResponse>.Wrap(true, response);

                return ResponseBase<UserNavResponse>.Wrap(false);
            }
        }
    }
}