...
using EnumerableExtension = Pixeval.Extensions.EnumerableExtension;
    ...
    public class UserFollowingAsyncEnumerable : AbstractPixivAsyncEnumerable<User>
    {
        ...
        private class UserFollowingAsyncEnumerator : AbstractPixivAsyncEnumerator<User>
        {
            ...
            protected override void UpdateEnumerator()
            {
                ...
                {
                    Thumbnails = EnumerableExtension.NonNull<FollowingResponse.Illust>(u.Illusts).Select(_ => _.ImageUrls.SquareMedium).ToArray(),
            ...
            private static async Task<ResponseBase<FollowingResponse>> TryGetResponse(string url)
            {
                var res = (await HttpClientFactory.AppApiHttpClient().GetStringAsync(url)).FromJson<FollowingResponse>();
                if (res is { } response && !response.UserPreviews.IsNullOrEmpty()) return ResponseBase<FollowingResponse>.Wrap(true, response);

                return ResponseBase<FollowingResponse>.Wrap(false);
            }
        }
    }
}