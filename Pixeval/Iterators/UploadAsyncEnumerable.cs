...
namespace Pixeval.Iterators
{
    public class UploadAsyncEnumerable : AbstractPixivAsyncEnumerable<Illustration>
    {
        ...
        private class UploadAsyncEnumerator : AbstractPixivAsyncEnumerator<Illustration>
        {
            ...
            private static async Task<ResponseBase<UploadResponse>> TryGetResponse(string url)
            {
                ...
                if (res is { } response && !response.Illusts.IsNullOrEmpty()) return ResponseBase<UploadResponse>.Wrap(true, response);

                return ResponseBase<UploadResponse>.Wrap(false);
            }
        }
    }
}