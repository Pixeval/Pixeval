...
namespace Pixeval.Iterators
{
    public class QueryAsyncEnumerable : AbstractPixivAsyncEnumerable<Illustration>
    {
        ...
        private class QueryAsyncEnumerator : AbstractPixivAsyncEnumerator<Illustration>
        {
            ...
            private static async Task<ResponseBase<QueryWorksResponse>> TryGetResponse(string url)
            {
                ...
                if (res is { } response && !response.Illusts.IsNullOrEmpty()) return ResponseBase<QueryWorksResponse>.Wrap(true, response);

                return ResponseBase<QueryWorksResponse>.Wrap(false);
            }
        }
    }
}