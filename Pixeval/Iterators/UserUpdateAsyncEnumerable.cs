...
namespace Pixeval.Iterators
{
    public class UserUpdateAsyncEnumerable : AbstractPixivAsyncEnumerable<Illustration>
    {
        ...
        private class UserUpdateAsyncEnumerator : AbstractPixivAsyncEnumerator<Illustration>
        {
            ...
            private static async Task<ResponseBase<UserUpdateResponse>> TryGetResponse(string url)
            {
                var res = (await HttpClientFactory.AppApiHttpClient().GetStringAsync(url)).FromJson<UserUpdateResponse>();
                if (res is { } response && !response.Illusts.IsNullOrEmpty()) return ResponseBase<UserUpdateResponse>.Wrap(true, response);

                return ResponseBase<UserUpdateResponse>.Wrap(false);
            }
        }
    }
}