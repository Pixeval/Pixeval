...
namespace Pixeval.Iterators
{
    public class RankingAsyncEnumerable : AbstractPixivAsyncEnumerable<Illustration>
    {
        ...
        private class RankingAsyncEnumerator : AbstractPixivAsyncEnumerator<Illustration>
        {
            ...
            private static async Task<ResponseBase<RankingResponse>> TryGetResponse(string url)
            {
                var res = (await HttpClientFactory.AppApiHttpClient().GetStringAsync(url)).FromJson<RankingResponse>();
                if (res is { } response && !response.Illusts.IsNullOrEmpty()) return ResponseBase<RankingResponse>.Wrap(true, response);

                return ResponseBase<RankingResponse>.Wrap(false);
            }
        }
    }
}