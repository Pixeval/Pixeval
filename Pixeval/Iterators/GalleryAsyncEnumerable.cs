...
namespace Pixeval.Iterators
{
    public class GalleryAsyncEnumerable : AbstractPixivAsyncEnumerable<Illustration>
    {
        ...
        private class GalleryAsyncEnumerator : AbstractPixivAsyncEnumerator<Illustration>
        {
            ...
            private static async Task<ResponseBase<GalleryResponse>> TryGetResponse(string url)
            {
                ...
                if (result is { } response && !response.Illusts.IsNullOrEmpty()) return ResponseBase<GalleryResponse>.Wrap(true, response);
                return ResponseBase<GalleryResponse>.Wrap(false);
            }
        }
    }
}