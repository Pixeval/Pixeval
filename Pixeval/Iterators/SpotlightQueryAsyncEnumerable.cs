...
namespace Pixeval.Iterators
{
    public class SpotlightQueryAsyncEnumerable : AbstractPixivAsyncEnumerable<SpotlightArticle>
    {
        ...
        private class SpotlightArticleAsyncEnumerator : AbstractPixivAsyncEnumerator<SpotlightArticle>
        {
            ...
            private async Task<ResponseBase<SpotlightResponse>> TryGetResponse()
            {
                var res = await HttpClientFactory.AppApiService().GetSpotlights(current++ * 10);

                if (res is { } response && !response.SpotlightArticles.IsNullOrEmpty()) return ResponseBase<SpotlightResponse>.Wrap(true, response);

                return ResponseBase<SpotlightResponse>.Wrap(false);
            }
        }
    }
}