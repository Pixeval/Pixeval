...
- 不要为 `en-US`、`fr-FR`、`ru-RU` 或其他非默认语言新增字符串、补翻译或创建缺失的资源文件。新增、修改、迁移或删除资源时，非默认语言只删除对应的资源条目；若父对象因此为空则一并删除空对象，保留无关的既有内容。
...
2. 将通用文案放入默认语言下合适的共享资源文件，只在 `zh-Hans` 新增或修改中文值，并在所有存在对应条目的非默认语言文件中删除该条目；若父对象因此为空则一并删除空对象。
